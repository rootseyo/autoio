...
- 저장한 좌표 또는 3초 뒤 현재 커서 위치에서 오토 클릭
- 오토 클릭 반복 횟수와 초당 클릭 수(CPS) 설정, `F10` 시작/중지
- 반복 재생 및 `-1` 무한 반복
- 매크로 저장, 불러오기, 가져오기, 삭제
...
1. **Saved position** 또는 **Cursor after 3s** 위치 모드를 선택합니다.
2. **Saved position**은 X/Y 좌표를 직접 입력하거나 **Capture in 3s**를 누른 뒤 원하는 위치로 커서를 옮깁니다.
3. **Cursor after 3s**는 시작 후 3초 안에 원하는 위치로 커서를 옮기면 그 위치를 사용합니다.
4. **Clicks**에 반복 횟수를 입력합니다. 무한 반복은 `-1`입니다.
5. **Speed (CPS)**에 초당 클릭 횟수를 `0.1~100` 사이로 입력합니다.
6. **Start auto click** 또는 `F10`으로 시작하고, 다시 누르면 즉시 중지합니다.

안전을 위해 AutoIO 창 내부 위치는 오토 클릭 대상으로 사용할 수 없으며, 녹화·매크로 재생·오토 클릭은 동시에 실행되지 않습니다.
